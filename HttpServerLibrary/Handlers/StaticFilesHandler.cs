...
public class StaticFilesHandler : Handler
{
    ...
    {
        Console.WriteLine($"Handling request of type {context.Request.HttpMethod} for {context.Request.Url?.AbsolutePath}");
        ...
        Console.WriteLine($"is GET {isGet} & is FILE {isFile}");
        if (isGet && isFile)
        {
            try
            {
